...
| Ver arquivo enviado | Só para os documentos **enviados nesta sessão** (o `File` fica em memória — não é persistido, fato d — e o acervo semeado não tem arquivo). Ao expandir a linha, "Ver arquivo enviado" abre o próprio arquivo num modal: imagem com girar (fato b — foto torta) e tamanho real; PDF no leitor do navegador. É a versão simples — não o visualizador lado a lado com os campos e zoom de precisão, que fica para a fatia da conferência |
