...
| Ver arquivo enviado | Só para os documentos **enviados nesta sessão** (o `File` fica na sessão — memória + sessionStorage, fato d — e o acervo semeado não tem arquivo). Abre o próprio arquivo num modal: imagem com girar (fato b — foto torta) e tamanho real; PDF no leitor do navegador; `.txt` como texto. DOC/DOCX/ODT/RTF/TIFF/HEIC o navegador não abre sozinho — aviso para abrir no aplicativo próprio. Aparece em "Processados" (ao expandir a linha) **e em "Resultados"** — nesta, importa no caso de conferência: a pessoa abre o documento original para corrigir o que a máquina errou. Não é o visualizador lado a lado com os campos e zoom de precisão, que fica para a fatia da conferência |
